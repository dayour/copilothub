...
describe('MicrosoftAppPanel', () => {
  ...
  it('surfaces browser fallback limitations explicitly', () => {
    ...
    expect(screen.getByText('Companion window required for Microsoft maker surfaces')).toBeInTheDocument();
    expect(screen.getByText('Launch Power Platform')).toBeInTheDocument();
    expect(screen.getByText('Open companion window')).toBeInTheDocument();
  });
  ...
  it('falls back to the curated Microsoft URL when a panel tab receives an untrusted destination', () => {
    ...
    expect(openSpy).toHaveBeenCalledWith(
      POWER_APPS_URL,
      'copilothub-power-platform-power-panel',
