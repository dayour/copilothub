...
export function ProjectSidebar({ onAddProject }: ProjectSidebarProps) {
  ...
  const chatTabId = useMemo(
    () => tabs.find((tab) => tab.type === 'chat')?.id ?? null,
    [tabs],
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 p-2">
        ...
        <div className={`rounded-lg border border-border-default bg-surface-tertiary/60 ${projectSidebarCollapsed ? 'p-1.5' : 'p-2'}`}>
          {!projectSidebarCollapsed && (
            <div className="mb-2 px-1 text-[11px] uppercase tracking-wide text-text-muted">
              Quick Launch
            </div>
          )}

          <div className={projectSidebarCollapsed ? 'flex flex-col gap-1.5' : 'grid grid-cols-2 gap-2'}>
            {quickLaunchItems.map((item, index) => {
              const Icon = item.icon;
              const isWideItem = !projectSidebarCollapsed && index === quickLaunchItems.length - 1;
              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-md border border-border-default bg-surface-elevated px-3 py-2 text-[12px] font-medium text-text-primary transition-colors hover:bg-surface-hover ${
                    projectSidebarCollapsed ? 'justify-center px-0' : 'justify-start'
                  } ${isWideItem ? 'col-span-2' : ''}`}
                  aria-label={`Open ${item.label}`}
                  title={projectSidebarCollapsed ? item.label : `Open ${item.label}`}
                >
                  <Icon size={15} className="shrink-0 text-text-secondary" />
                  {!projectSidebarCollapsed && <span>{item.label}</span>}
                </button>
              );
            })}
          </div>
