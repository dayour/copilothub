...
describe('VSCodeTab', () => {
  ...
  it('renders a practical local extension manager beside the fallback workbench', async () => {
    ...
    expect(screen.getByText('VS Code remote fallback opens in a separate browser tab')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'Open VS Code' })).toHaveAttribute('href', 'https://vscode.dev');
