...
import { usePcaeStore } from '../store/pcaeStore';
import type {
  GraphCalendarEvent,
  GraphDriveItem,
  GraphMailMessage,
...
// ---------------------------------------------------------------------------
// WorkIqSidebarWidget -- compact Work IQ query widget for the Copilot sidebar
// ---------------------------------------------------------------------------

function WorkIqSidebarWidget() {
  const [open, setOpen] = useState(false);
  const [question, setQuestion] = useState('');
  const [isQuerying, setIsQuerying] = useState(false);
  const [answer, setAnswer] = useState<string | null>(null);
  const [queryError, setQueryError] = useState<string | null>(null);
  const pendingCount = usePcaeStore((s) => s.pendingApprovalTasks().length);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (result.success && result.response) {
      ...
      setQueryError(result.error ?? 'Work IQ error');
    }
  };
  ...
  return (
    <div className="px-3 py-3 border-b border-border-default">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 w-full text-left text-[11px] uppercase tracking-wide text-text-muted hover:text-text-secondary transition-colors"
        aria-expanded={open}
      >
        ...
        <span>Work IQ</span>
        {pendingCount > 0 && (
          <span className="ml-auto rounded-full bg-yellow-500 px-1.5 py-0.5 text-[10px] font-bold leading-none text-black">
      ...
      {open && (
        <div className="mt-2 flex flex-col gap-2">
          <form onSubmit={handleSubmit} className="flex gap-1.5">
            <input
              type="text"
              className="min-w-0 flex-1 h-8 rounded-md px-2.5 text-[12px] bg-surface-tertiary text-text-primary placeholder:text-text-muted border border-border-subtle focus:outline-none focus:border-accent-primary"
            ...
            <button
              ...
              className="h-8 px-2.5 rounded-md text-[12px] font-medium bg-accent-primary text-white hover:bg-accent-hover disabled:opacity-40 transition-colors"
            >
              {isQuerying ? <RefreshCw size={12} className="animate-spin" /> : <Zap size={12} />}
            </button>
          </form>
          {queryError && (
            <p className="text-[11px] text-status-error leading-relaxed">{queryError}</p>
          )}
          {answer && (
            <div className="rounded-md bg-surface-tertiary/70 border border-border-subtle p-2 text-[12px] text-text-primary leading-relaxed max-h-40 overflow-y-auto whitespace-pre-wrap">
              {answer}
            </div>
          )}
        </div>
