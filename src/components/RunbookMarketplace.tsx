...
export function RunbookMarketplace({
  ...
  const content = (
    <section className="flex h-full w-full flex-col gap-4 rounded-lg border border-border-default bg-surface-secondary p-4">
      ...
      {!isTauri && !error ? (
        <p className="rounded-md border border-[var(--color-status-warning)]/40 bg-[var(--color-status-warning)]/10 px-3 py-2 text-sm text-[var(--color-status-warning)]">
          Browser-host mode can browse the marketplace shell, but runbook files are only persisted inside the
          CopilotHub Tauri desktop runtime.
        </p>
      ) : null}
