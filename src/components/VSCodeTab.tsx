  ...
  const handleStartLocalServer = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Refresh failure after an error is non-fatal; original error already captured above.
      }
    } finally {
      setIsStarting(false);
    }
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <div className="relative flex-1 overflow-hidden">
          ...
          ) : requiresExternalBrowserFallback ? (
            <div className="flex h-full items-center justify-center bg-surface-primary p-8">
              <div className="max-w-xl rounded-xl border border-white/10 bg-surface-secondary/60 p-6 text-left">
                <div className="text-base font-semibold text-text-primary">
                  VS Code remote fallback opens in a separate browser tab
                </div>
                <p className="mt-2 text-sm text-text-secondary">
                  {APP_CONFIG.vsCodeUrl} blocks embedding via iframe in browser-host mode. Open it in a new tab, or
