...
export type TabType = 'browser' | 'browser-use' | 'chat' | 'vscode' | 'terminal' | 'runbook' | 'copilot-studio' | 'power-platform' | 'cowork' | 'demo-calendar' | 'demo-mechanic' | 'demo-coffeeshop' | 'demo-adaptive-cards' | 'demo-wiring' | 'demo-studio-guide' | 'demo-media-assets' | 'demo-animations' | 'showcase-coffee' | 'showcase-clothing' | 'showcase-insurance' | 'showcase-it-helpdesk' | 'showcase-seller' | 'showcase-fleet' | 'showcase-fuel' | 'showcase-dayour-protocol';
...
export const useTabStore = create<TabStore>()(
  immer((set, get) => ({
    ...
    syncTabUrlFromHost: (id: string, url: string) => {
      set((state) => {
        const tab = state.tabs.find((t) => t.id === id);
        if (!tab || (tab.type !== 'browser' && !isMicrosoftPanelTab(tab.type))) return;
        ...
        const previousUrl = tab.historyIndex > 0 ? tab.historyStack[tab.historyIndex - 1] : null;
        if (previousUrl === url) {
          tab.historyIndex -= 1;
          tab.url = url;
          refreshHistoryFlags(tab);
        ...
        const nextUrl = tab.historyIndex < tab.historyStack.length - 1
          ? tab.historyStack[tab.historyIndex + 1]
          : null;
        if (nextUrl === url) {
          tab.historyIndex += 1;
          tab.url = url;
