...
describe('CommandPalette', () => {
  ...
  it('opens settings from the command palette', () => {
    resetAppStore({ commandPaletteOpen: true });
    render(<CommandPalette />);
  ...
  it('opens Copilot Studio panel from the command palette', () => {
    resetAppStore({ commandPaletteOpen: true });
    render(<CommandPalette />);

    fireEvent.click(screen.getByText('Open Copilot Studio Panel'));

    const newestTab = useTabStore.getState().tabs.at(-1);
    expect(newestTab?.type).toBe('copilot-studio');
    expect(useAppStore.getState().commandPaletteOpen).toBe(false);
  });

  it('opens Power Automate maker destination from the command palette', () => {
    resetAppStore({ commandPaletteOpen: true });
    render(<CommandPalette />);

    fireEvent.click(screen.getByText('Open Power Automate Maker'));

    const newestTab = useTabStore.getState().tabs.at(-1);
    expect(newestTab?.type).toBe('power-platform');
    expect(newestTab?.url).toBe(POWER_AUTOMATE_URL);
  });

  it('opens the Git review pane from the command palette', () => {
    resetAppStore({ commandPaletteOpen: true, copilotSidebarOpen: false, assistantPaneMode: 'copilot' });
    render(<CommandPalette />);

    fireEvent.click(screen.getByText('Open Git Review Pane'));
