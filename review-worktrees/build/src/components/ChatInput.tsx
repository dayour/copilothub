...
export function ChatInput() {
  ...
  const inputDraft = useChatStore(
    (s) => s.threadStateById[selectedThreadId ?? 'default-thread']?.inputDraft ?? '',
  );
  ...
  const mode = useChatStore(
    (s) => s.threadStateById[selectedThreadId ?? 'default-thread']?.mode ?? 'chat',
  );
  ...
  function handleInputChange(e: React.ChangeEvent<HTMLTextAreaElement>) {
    ...
    setInputDraft(value, selectedThreadId);
  ...
  function insertMention(mention: MentionTarget) {
    ...
    setInputDraft(newText, selectedThreadId);
    setActiveMention(mention, selectedThreadId);
    setShowMentions(false);
    setMentionCtx(null);
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (e.key === 'Escape') {
      setInputDraft('', selectedThreadId);
      setActiveMention(null, selectedThreadId);
    }
  }
  ...
  return (
    <div className="relative bg-surface-secondary border-t border-border-default">
      ...
      <div className="flex items-center gap-1 px-3 pb-2">
        <button
          type="button"
          onClick={() => setMode('chat', selectedThreadId)}
        ...
        <button
          type="button"
          onClick={() => setMode('action', selectedThreadId)}
          className={`px-3 py-1 rounded text-[12px] font-medium transition-colors ${
            mode === 'action'
              ? 'bg-accent-primary text-white'
              : 'bg-surface-tertiary text-text-secondary hover:text-text-primary hover:bg-surface-hover'
