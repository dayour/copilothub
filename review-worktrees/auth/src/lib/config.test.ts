...
describe('config validation', () => {
  ...
  describe('isValidUrl', () => {
    ...
    it('rejects non-URL strings', () => {
      ...
      expect(isValidUrl('http://localhost:bad-port')).toBe(false);
    });
  ...
  describe('isLocalhostUrl', () => {
    it('accepts localhost URLs used by local VS Code servers', () => {
      expect(isLocalhostUrl('http://localhost:8080')).toBe(true);
      expect(isLocalhostUrl('http://127.0.0.1:8080/healthz')).toBe(true);
      expect(isLocalhostUrl('http://[::1]:8080')).toBe(true);
    });

    it('rejects non-local URLs', () => {
      expect(isLocalhostUrl('https://vscode.dev')).toBe(false);
      expect(isLocalhostUrl('https://example.com')).toBe(false);
      expect(isLocalhostUrl('not-a-url')).toBe(false);
      expect(isLocalhostUrl('ftp://localhost:8080')).toBe(false);
    });
  });
  ...
  describe('validateAppConfig', () => {
    ...
    it('reports invalid vsCodeLocalUrl', () => {
      const errors = validateAppConfig({ ...APP_CONFIG, vsCodeLocalUrl: 'https://vscode.dev' as any });
      expect(errors.some((e) => e.field === 'vsCodeLocalUrl')).toBe(true);
    });

    it('reports invalid vsCodeLocalHealthUrl', () => {
      const errors = validateAppConfig({ ...APP_CONFIG, vsCodeLocalHealthUrl: 'ftp://localhost:8080' as any });
      expect(errors.some((e) => e.field === 'vsCodeLocalHealthUrl')).toBe(true);
    });

    it('reports invalid vsCodeLocalStartupTimeout', () => {
      const errors = validateAppConfig({ ...APP_CONFIG, vsCodeLocalStartupTimeout: 0 as any });
      expect(errors.some((e) => e.field === 'vsCodeLocalStartupTimeout')).toBe(true);
    });
    ...
    it('accumulates multiple errors', () => {
      const errors = validateAppConfig({
        ...
        vsCodeLocalStartupTimeout: 0 as any,
        maxTabs: -5 as any,
      });
