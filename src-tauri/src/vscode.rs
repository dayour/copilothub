...
fn handle_start_timeout_cleanup(
    state: &mut VsCodeServerState,
    config: &VsCodeServerConfig,
    pid: u32,
...
pub async fn vscode_server_start(
    ...
    Err(handle_start_timeout_cleanup(
        &mut state,
        &config,
        pid,
...
mod tests {
    ...
    fn sample_vscode_server_config() -> VsCodeServerConfig {
        VsCodeServerConfig {
            local_url: "http://127.0.0.1:8080".to_string(),
            health_url: "http://127.0.0.1:8080/healthz".to_string(),
            command: Some("code-server".to_string()),
            args: Vec::new(),
    ...
    #[test]
    fn timeout_cleanup_clears_tracked_pid_after_successful_shutdown() {
        let config = sample_vscode_server_config();
        let mut state = VsCodeServerState {
            pid: Some(4242),
            lifecycle: VsCodeServerLifecycle::Starting,
            last_error: None,
        };

        let error = handle_start_timeout_cleanup(&mut state, &config, 4242, Ok(()));

        assert_eq!(
            error,
            "Local VS Code server did not become reachable at http://127.0.0.1:8080/healthz within 25ms"
        );
        assert_eq!(state.pid, None);
        assert_eq!(
            state.lifecycle,
            VsCodeServerLifecycle::Error(error.clone())
        );
        assert_eq!(state.last_error.as_deref(), Some(error.as_str()));
    }

    #[test]
    fn timeout_cleanup_retains_pid_when_shutdown_fails() {
        let config = sample_vscode_server_config();
        let mut state = VsCodeServerState {
            pid: Some(7777),
            lifecycle: VsCodeServerLifecycle::Starting,
        ...
        let error = handle_start_timeout_cleanup(
            &mut state,
            &config,
            7777,
        ...
        assert_eq!(
            error,
            "Local VS Code server did not become reachable at http://127.0.0.1:8080/healthz within 25ms; additionally failed to stop PID 7777 after timeout: access denied"
        );
        assert_eq!(state.pid, Some(7777));
        assert_eq!(
            state.lifecycle,
            VsCodeServerLifecycle::Error(error.clone())
        );
